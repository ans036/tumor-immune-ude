...
\label{eq:total_loss}
\end{align}
where $\mathcal{L}_{\text{volume}} = \frac{1}{N}\sum_{g,j}(\hat{V}_g(t_{g,j}) - V^{\text{obs}}_g(t_{g,j}))^2$ is the mean squared error between predicted and observed tumor volumes across all $N$ data points, $\mathcal{L}_{\text{negativity}} = \sum_{g,j}\max(0, -\hat{V}_g(t_{g,j}))^2$ penalizes non-physical negative volume predictions, and $\mathcal{L}_{\text{reg}} = \|\boldsymbol{\theta}_{\text{nn}}\|_2^2$ is $L_2$ weight decay on neural network parameters. The weights $\lambda_{\text{vol}}=1.0$, $\lambda_{\text{phys}}=0.1$, $\lambda_{\text{neg}}=10^{-3}$, $\lambda_{\text{cons}}=0.5$, and $\lambda_{\text{reg}}=10^{-5}$ were selected by grid search on training loss convergence and biological consistency.
...
\caption{Parameter evolution trajectories for the representative case C4-T5, showing temporal dynamics of $r(t)$, $K(t)$, $c_{\text{kill}}(t)$, and $h_{\text{sat}}(t)$ with enlarged fonts for readability.}
